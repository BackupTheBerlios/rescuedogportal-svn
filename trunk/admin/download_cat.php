...
if(!$action) {
	...
	$i=0;
	$result = $db->query("SELECT *	FROM rhs_download_cat WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." ORDER BY $orderby $orderdir");
	...
	$smarty->display("a_download_cat_show.tpl.php");
	exit();
}
...
if($action=="edit") {
	...
	if($_POST['send']){
		$result = $db->query("UPDATE rhs_download_cat SET catname='$_POST[catname]', catorder='$_POST[catorder]' WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
		if($result){
	...
	$rowlink = $db->query_first("SELECT * FROM rhs_download_cat WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
	$smarty->assign($rowlink);
	$smarty->display("a_download_cat_edit.tpl.php");
	exit();
}
...
if($action=="new") {
	$smarty->assign("action", $action);
	if($_POST['send']){
		$result = $db->query("INSERT INTO rhs_download_cat SET catname='$_POST[catname]', catorder='$_POST[catorder]', mandant=".$adminsession->session_mandant_data['mandant_id'].", create_userid=".$adminsession->session_user_data['customer_id'].", create_date=".time());
		if($result){
			$rowlink = $db->query_first("SELECT * FROM rhs_download_cat WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=".$db->insert_id());
			$smarty->assign("fehler", 1);
			$smarty->assign($rowlink);
			$smarty->assign("page_redirect", "download_cat.php?sid=".$adminsession->session_data['hash']);
		}else $smarty->assign("fehler", 2);
...
**/
if($action=="deleteref") {
	$result = $db->query("SELECT catname, id FROM rhs_download_cat WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=".$_GET['id']);
	$catname = $db->fetch_array($result);
	$smarty->assign("deletename", $catname['catname']);
	$smarty->assign("deleteid", $catname['id']);
	$smarty->assign("action", $action);
	$smarty->assign("filename", $filename);
	$smarty->assign("pagename", "Download Cat");

	/* Wenn das Forumlar abgeschickt wurde */
	if(isset($_POST['send'])) {
		if(isset($_POST['ja'])){
			$db->query("DELETE FROM rhs_download_cat WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=".$_GET['id']);
			$db->query("DELETE FROM rhs_download_data WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND catid=".$_GET['id']);
		}
		header("Location: download.php?sid=".$adminsession->session_data['hash']);
		exit();
	}
