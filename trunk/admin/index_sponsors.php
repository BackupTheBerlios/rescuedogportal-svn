...
*
*
* Filename: index_sponsors.php
*
* @author Markus Wilhelm <markus.wilhelm@rescue-dogs.de>
* @version 1.1
* @access public
* @package BRKsponsorsAdmin
...
*/
require("_global.php");
if($adminsession->session_user_data['admin_can_use_index_sponsors'] !=1)$adminsession->NoEntryForUser();
...
if(!$action) {
	...
	$i=0;
	$result = $db->query("SELECT *	FROM rhs_sponsors WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." ORDER BY $orderby $orderdir");
	...
	$smarty->display("a_index_sponsors_show.tpl.php");
	exit();
}
...
if($action=="edit") {
	...
	if($_POST['send']){
		$result = $db->query("UPDATE rhs_sponsors SET url='$_POST[url]', email='$_POST[email]', text='$_POST[message]', bilddb_id='$_POST[bilddb_id]', titel='$_POST[titel]', changedate=".time()." WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
		if($result){
			$smarty->assign("fehler", 1);
			$smarty->assign("page_redirect", "index_sponsors.php?sid=".$adminsession->session_data['hash']);
		}else $smarty->assign("fehler", 2);
	}
	$rowlink = $db->query_first("SELECT * FROM rhs_sponsors WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
	...
	$smarty->display("a_index_sponsors_edit.tpl.php");
	exit();
}
...
if($action=="new") {
	$smarty->assign("action", $action);
	if($_POST['send']){
		$result = $db->query("INSERT INTO rhs_sponsors SET url='$_POST[url]', email='$_POST[email]', text='$_POST[message]', bilddb_id='$_POST[bilddb_id]', titel='$_POST[titel]', mandant=".$adminsession->session_mandant_data['mandant_id'].", create_userid=".$adminsession->session_user_data['customer_id'].", create_date=".time());
		if($result){
			$rowlink = $db->query_first("SELECT * FROM rhs_sponsors WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=".$db->insert_id());
			$smarty->assign("fehler", 1);
			$smarty->assign("page_redirect", "index_sponsors.php?sid=".$adminsession->session_data['hash']);
		}else $smarty->assign("fehler", 2);
	}
...
**/
if($action=="delete") {
	$result = $db->query("SELECT titel, id FROM rhs_sponsors WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
	$catname = $db->fetch_array($result);
	$smarty->assign("deletename", $catname['titel']);
	$smarty->assign("deleteid", $catname['id']);
	$smarty->assign("action", $action);
	$smarty->assign("filename", $filename);
	$smarty->assign("pagename", "sponsorsnews");

	/* Wenn das Forumlar abgeschickt wurde */
	if(isset($_POST['send'])) {
		if(isset($_POST['ja'])){
			$db->query("DELETE FROM rhs_sponsors WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
		}
		header("Location: index_sponsors.php?sid=".$adminsession->session_data['hash']);
		exit();
	}
