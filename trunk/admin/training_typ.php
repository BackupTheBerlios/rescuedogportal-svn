...
if(!$action) {
	...
	$i=0;
	$result = $db->query("SELECT *	FROM rhs_training_training WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." ORDER BY $orderby $orderdir");
	...
	$smarty->display("a_training_typ_show.tpl.php");
	exit();
}
...
if($action=="edit") {
	...
	if($_POST['send']){
		$result = $db->query("UPDATE rhs_training_training SET training='$_POST[p_training]' WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
		if($result){
			$smarty->assign("fehler", 1);
			$smarty->assign("page_redirect", "training_typ.php?sid=".$adminsession->session_data['hash']);
		}else $smarty->assign("fehler", 2);
	}
	$rowlink = $db->query_first("SELECT * FROM rhs_training_training WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
	$smarty->assign($rowlink);
	$smarty->display("a_training_typ_edit.tpl.php");
	exit();
}
...
if($action=="new") {
	$smarty->assign("action", $action);
	if($_POST['send']){
		$result = $db->query("INSERT INTO rhs_training_training SET training='$_POST[p_training]', mandant=".$adminsession->session_mandant_data['mandant_id'].", create_userid=".$adminsession->session_user_data['customer_id'].", create_date=".time());
		if($result){
			$rowlink = $db->query_first("SELECT * FROM rhs_training_training WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=".$db->insert_id());
			$smarty->assign("fehler", 1);
			$smarty->assign($rowlink);
			$smarty->assign("page_redirect", "training_typ.php?sid=".$adminsession->session_data['hash']);
		}else $smarty->assign("fehler", 2);
...
**/
if($action=="delete") {
	$result = $db->query("SELECT training, id FROM rhs_training_training WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
	$catname = $db->fetch_array($result);
	$smarty->assign("deletename", $catname['training']);
	$smarty->assign("deleteid", $catname['id']);
	$smarty->assign("action", $action);
	$smarty->assign("filename", $filename);
	$smarty->assign("pagename", "Trainings Typ");

	/* Wenn das Forumlar abgeschickt wurde */
	if(isset($_POST['send'])) {
		if(isset($_POST['ja'])){
			$db->query("DELETE FROM rhs_training_training WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
		}
		header("Location: training_typ.php?sid=".$adminsession->session_data['hash']);
		exit();
	}
