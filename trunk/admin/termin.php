...
if($adminsession->session_user_data['admin_can_use_termin'] !=1)$adminsession->NoEntryForUser();
...
if(!$action) {
	...
	$i=0;
	$result = $db->query("SELECT *	FROM rhs_tberichte WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." ORDER BY $orderby $orderdir");
	...
	$smarty->display("a_termin_show.tpl.php");
	exit();
}
...
if($action=="edit") {
	...
	if($_POST['send']){
		$result = $db->query("UPDATE rhs_tberichte SET titel='$_POST[titel]', datefrom='".makemytime($_POST['datum'])."', dateto='".makemytime($_POST['datumbis'])."', ort='$_POST[ort]', beschreibung='$_POST[message]' WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
		if($result){
	...
	$rowlink = $db->query_first("SELECT * FROM rhs_tberichte WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
...
**/
if($action=="delete") {
	$result = $db->query("SELECT titel, id FROM rhs_tberichte WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
	$catname = $db->fetch_array($result);
	$smarty->assign("deletename", $catname['titel']);
	$smarty->assign("deleteid", $catname['id']);
	$smarty->assign("action", $action);
	$smarty->assign("filename", $filename);
	$smarty->assign("pagename", "Termin");

	/* Wenn das Forumlar abgeschickt wurde */
	if(isset($_POST['send'])) {
		if(isset($_POST['ja'])){
			$db->query("DELETE FROM rhs_tberichte WHERE mandant=".$adminsession->session_mandant_data['mandant_id']." AND id=$_GET[id]");
		}
		header("Location: termin.php?sid=".$adminsession->session_data['hash']);
		exit();
	}
