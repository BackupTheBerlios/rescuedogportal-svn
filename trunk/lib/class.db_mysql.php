...
class db {
 ...
 function query($query_string,$limit=0,$offset=0) {
  global $query_count,$querytimer,$pagestarttime;
  if($limit!=0) $query_string.=" LIMIT $offset, $limit";
  $this->queries[]="$query_string";
  if ($pagestarttime) {
  	$startsqltime=microtime();
  	$this->query_id = mysql_query($query_string,$this->link_id);
  	$querytimer+=microtime_past($startsqltime,microtime());
  	$query_count++;
  } else $this->query_id = mysql_query($query_string,$this->link_id);
  if (!$this->query_id) $this->error("Invalid SQL: ".$query_string);
  return $this->query_id;
 }
 
function unbuffered_query($query_string,$LOW_PRIORITY=0) {
global $query_count,$querytimer,$pagestarttime,$showqueries;
...
} else $this->query_id = mysql_unbuffered_query($query_string,$this->link_id);
if (!$this->query_id) $this->error("Invalid SQL: ".$query_string);
return $this->query_id;
}
}
