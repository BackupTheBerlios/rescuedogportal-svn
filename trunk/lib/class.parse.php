...
class parse {
	...
	function getsmilies() {
		global $db, $n, $lang;
		$i = 0;
		$result = $db->unbuffered_query("SELECT smilieid, smiliecode, smiliepath, smilietitle FROM rhs_smilies ORDER BY smilieorder ASC");
		while ($row = $db->fetch_array($result)) {
			$row['smilietitle'] = $row['smilietitle'];
	...
	function getbbcode() {
		...
		$result = $db->unbuffered_query("SELECT bbcodetag, bbcodereplacement, params, multiuse, pattern1, pattern2, pattern3, eval_replacement FROM rhs_bbcodes ORDER BY params ASC");
	...
	function codeformat($code, $mode) {
		...
		if ($mode == "php") {
			...
			if (!wbb_strpos($code, "<?") && substr($code, 0, 2) != "<?") {
				$phptags = 1;
				$code = "<?php ".trim($code)." ?>";
			}
			ob_start();
	...
	function formaturl($url, $title = '', $maxwidth = 60, $width1 = 40, $width2 = -15) {
		if (!trim($title)) {
			...
			if ($this->cuturls == 1 && wbb_strlen($title) > $maxwidth) $title = substr($title, 0, $width1)."...".substr($title, $width2);
