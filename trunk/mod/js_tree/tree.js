...
function item_get_icon (b_junction) {
	...
	if(b_junction)
	{
		...
		//document.getElementsByTagName("p")[0].firstChild.data = document.getElementsByTagName("p")[0].firstChild.data + "-" + this.a_children.length;
		// has children
...
function item_get_data() {
 ...
 if(this.a_config[1]=='3') {
  ...
  return get_img_tag("moderator") + this.a_config[0] + " <span class=\"small\"><a href=\"board.php?action=editmoderator&amp;boardid=" + node_data[0] + "&amp;create_userid=" + node_data[1] + "&amp;sid=" + session_hash + "\">[" + lang_var_edit + "]</a> <a href=\"board.php?action=delmoderator&amp;boardid=" + node_data[0] + "&amp;create_userid=" + node_data[1] + "&amp;sid=" + session_hash + "\">[" + lang_var_del + "]</a></span>";
 }
 
 // permissions
 if(this.a_config[1]=='4') return get_img_tag("forum") + "<i>" + this.a_config[0] + "</i>";
 ...
 if(node_data[2]==0){
  ...
  return get_img_tag(image) + "<span style=\"color: " + color + "\">" + this.a_config[0] + "</span> <span class=\"small\"><a href=\"" + node_data[0] + "?sid=" + session_hash + "\">[Anzeigen]</a> <a href=\"" + node_data[0] + "?action=new&sid=" + session_hash + "\">[Erstellen]</a></span>";
 }
 
  // usergroup
 if(this.a_config[1]=='51') {
  node_data = this.a_config[2].split("|");
  ...
  return get_img_tag(image) + "<span style=\"color: " + color + "\">" + this.a_config[0] + "</span> <span class=\"small\"><a href=\"" + node_data[0] + "?sid=" + session_hash + "\">[Anzeigen]</a></span>";
 }
 
   // usergroup
  if(this.a_config[1]=='52') return get_img_tag("permissions") + "<i><a href=\"index.php?simple=1&sid=" + session_hash + "\">" + this.a_config[0] + "</a></i>";
  
  if(this.a_config[1]=='53') return get_img_tag("permissions") + "<i><a href=\"\" onClick=\"javascript:window.open('../mod/poll/admin/index.php?sid=" + session_hash +"', 'Poll Admin', 'width=850,height=500,left=0,top=0,scrollbars=yes,resizable=yes');\">" + this.a_config[0] + "</a></i>";
  

}

function get_img_tag(icon) {
 return "<img src=\"bilder/tree/" + icon + ".gif\" alt=\"\" border=\"0\" align=\"absbottom\" />";	
}
...
var tree_tpl = {
	...
	'icon_e'  : 'bilder/tree/empty.gif', // empty image
	'icon_l'  : 'bilder/tree/line.gif',  // vertical line
	
	'icon_2'  : 'bilder/tree/joinbottom.gif', // junction for leaf
	'icon_3'  : 'bilder/tree/join.gif',       // junction for last leaf
	'icon_18' : 'bilder/tree/plusbottom.gif', // junction for closed node
	'icon_19' : 'bilder/tree/plus.gif',       // junctioin for last closed node
	'icon_26' : 'bilder/tree/minusbottom.gif',// junction for opened node
	'icon_27' : 'bilder/tree/minus.gif'       // junctioin for last opended node
};